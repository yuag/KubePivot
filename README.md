...
| 维度 | **SK5 + K8s** | **云平台 + SK5**（Cloud Run 等） |
|------|:-------------:|:------------------------------:|
| 隧道用途 | 访问 **K8s API**（内网 IP） | 访问 **metadata / HTTP**、隧道内网络 |
| 核心凭证 | **Bearer Token** + API Server | **printenv**、metadata（常不稳定） |
| 资源树 | ✅ | ❌ |
| Pod / 容器终端 | ✅ | ❌（无 Pod；纯 SK5 不能 shell） |
| 云平台命令 | — | ✅ |
| RBAC 审计 | ✅ | ❌ |
